...
func save_game(cps: int, cpc: int, level: int, iq: int) -> void:
...
func load_game() -> Dictionary:
	...
	if file:
		...
		file.close()
		if saveData != null and saveData.has("cps") and saveData.has("cpc") and saveData.has("level") and saveData.has("iq"):
			return saveData
		else:
